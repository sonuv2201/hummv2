...
const StepSix = ({resendAadharOtp,refreshProcess,fromProcess,setSignUpForm,aadharOtp,setAadhaar,setAadharOtp,createAadharSession,setMessage,token,verifyAadharOtp,message,successMessage,resend,timerVisible,timer}) => {

  const restartAadharProcess = () =>{
    setAadhaar("");
    setAadharOtp("");
  ...
  return (
    ...
    <div className="relative mb-5">
      <p className='text-[#000] opacity-1 text-[20] underline cursor-pointer' onClick={() => restartAadharProcess()}>Change Aadhar number</p>
    </div>
    ...
    { message === "" ? <></> : <>
      ...
      <button className='mt-3 font-bold underline' onClick={()=> restartAadharProcess()}>Click here to restart process</button>
    </>}
